...
class TriggerEngine:
    ...
    def _eval_position(self, row, price: float) -> None:
        ...
        # time-exit: автозакрытие после max_hold_seconds (HOLD_BARS из brain)
        hit_time = (order.max_hold_seconds is not None and
                    (time.time() - (row["created_at"] or 0)) >= order.max_hold_seconds)
        if not (hit_tp or hit_sl or hit_trail or hit_time):
            return

        # атомарный claim pending→exiting: только победитель шлёт sell (C4)
        if not self.store.claim_trigger(row["id"], "pending", "exiting"):
            return
        reason = "TP" if hit_tp else "SL" if hit_sl else "TRAIL" if hit_trail else "TIME"
